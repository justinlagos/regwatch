...
export default async function ItemsPage({ searchParams }: Props) {
  ...
  let query = sb
    ...
    .select(`
      ...
      sources(id, name),
      classifications(impact_level, summary)
    `, { count: 'exact' })
    .eq('state', 'classified')
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
              ...
              {filtered.map((item: any) => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <Link href={`/items/${item.id}`} className="font-medium text-slate-800 hover:text-blue-600 line-clamp-2">
                        {item.title || 'Untitled'}
                      </Link>
                      {cls?.summary && (
      ...
      <div className="sm:hidden space-y-2">
        {filtered.length === 0 && (
          <div className="bg-white rounded-xl border border-gray-200 p-8 text-center text-gray-400">No items found</div>
        )}
        {filtered.map((item: any) => {
          const cls = item.classifications?.[0]
          const itemLevel = cls?.impact_level
